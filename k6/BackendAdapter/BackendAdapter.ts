...
export function LoginCliente(loginPayload: LoginPayload): AuthTokenDTO {
    let loginRes = http.post(`${BASE_URL}/auth/login/clientes`, JSON.stringify(loginPayload), { headers: { 'Content-Type': 'application/json' }, tags: { name: 'LoginCliente' }, });
...
export function LoginGerente(loginPayload: LoginPayload): AuthTokenDTO {
    let loginRes = http.post(`${BASE_URL}/auth/login/gerentes`, JSON.stringify(loginPayload), { headers: { 'Content-Type': 'application/json' }, tags: { name: 'LoginGerente' }, });
    check(loginRes, { 'Login bem-sucedido': (res) => res.status === 200 });
    if(loginRes.status !== 200) {
        console.log(loginRes.error_code);
        console.log(loginPayload);
...
function authHeaders(authToken: string, functionName: string) {
    return { headers: { 'Authorization': `Bearer ${authToken}`, 'Content-Type': 'application/json' },  tags: { name: functionName }, };
}
